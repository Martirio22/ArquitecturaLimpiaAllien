	...
	public Usuario guardar(Usuario usuario) {
		...
		// 2. Validar Correo Único (Solo si es nuevo usuario)
			if (usuario.getIdUsuario() == null) {
				usuarioRepositorio.buscarPorCorreo(usuario.getCorreoElectronico()).ifPresent(u -> {
					throw new RuntimeException("El correo ya está registrado");
				});
	
				// 3. Valores por defecto para NUEVO usuario
				usuario = new Usuario(null, usuario.getPrimerNombre(), usuario.getSegundoNombre(),
						usuario.getPrimerApellido(), usuario.getSegundoApellido(), usuario.getNombreUsuario(),
						usuario.getCorreoElectronico(), usuario.getCedula(), usuario.getClave(), 0, null, usuario.getRol(),
						true // activo por defecto e intentos 0
				);
		} else {
			// Lógica para EDITAR: Recuperar datos que no se deben perder
			Usuario existente = buscarPorId(usuario.getIdUsuario());
			usuario = new Usuario(existente.getIdUsuario(), usuario.getPrimerNombre(), usuario.getSegundoNombre(),
					usuario.getPrimerApellido(), usuario.getSegundoApellido(), usuario.getNombreUsuario(),
					usuario.getCorreoElectronico(), usuario.getCedula(), usuario.getClave(),
					existente.getIntentosActual(), existente.getUltimoAcceso(), usuario.getRol(),
